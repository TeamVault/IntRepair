...
public class IndBasicBlock implements IProgramTreeEntry {
	...
	public IndBasicBlock(int pthreadnr, IBasicBlock localNode, ProgramPath entryPath) {
	...
	public String toString() {
		...
		//return new String(" (" + Thread.currentThread().getId() + ") " + res);
		return res;
	}
	...
	public int hashCode() {
		if (hashAvailable) {
			...
			if (entryPath == null) {
				...
				hash = entryPath.toString().hashCode() ^ localNode.hashCode() + pthreadnr;							
			}		
			hashCache = hash;
			hashAvailable = true;
			return hash;
