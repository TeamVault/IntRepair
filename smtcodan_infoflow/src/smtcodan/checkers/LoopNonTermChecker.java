...
public class LoopNonTermChecker implements IPathObserver {
	...
	public void loopClosed(PathExplorer pe, IBasicBlock decnode, int stemLength, boolean firstClosed) throws InfiniteLoopException {
		...
		for (int i=0; i< loopSrcInts.size(); i++) {
			fp.append("(assert (= " + loopSrcInts.get(i).getSSAName() + " " + prevInts.get(i).getSSAName() + " ))");			
		}
		fp.append("\n");

		// 1. for single-path unnested loop: check context-free infinite (tautology)
		...
		if (!isMultiPath) {
			...
			while (worklist.size() > 0) {
				...
				if (!visited.contains(cand) && !partssas.get(0).contains(cand)) {
					...
					for (SymVarSSA dep : cand.getDependencies()) {
						if (!visited.contains(dep) && !partssas.get(0).contains(dep)) {
			...
			String skipUnsat = sbdec.getFormula() + "\n" + 
					"(assert (= " + sbdec.getSSAName() + " false ))\n";

			String query = logicDef + stemdefs.toString() + loopdefs + loopeqs + loopbodyadds + skipUnsat + checkSat + exit;
		...
		// 2. for single-path unnested loop: check context-free finite (fixed-point absence)
		// if always finite return
		if (!founderror && !isMultiPath) {
			String query = logicDef + stemdefs.toString() + loopdefs + loopeqs + loopbodyadds + fp + checkSat + exit;
			String reply = sv.queryReply(query).toString();	
		...
		if (!founderror) {
			String query = logicDef + stemdefs.toString() + loopdefs + stemeqs + loopeqs + fp + checkSat + exit;
			String reply = sv.queryReply(query).toString();	
			if (reply.contains("unsat")) {
				founderror = false;
			} else {
		...
		if (founderror) {
			// found error
			String problemMessage = new String("Infinite loop");
			...
			System.out.println(" PathVal: found sat infinite loop (thread " + Thread.currentThread().getId() + " )");
			pe.reportProblem(problemid, file, startline, problemMessage, problemDescription);

			System.out.println("   PathVal: Path unsat due to INFINITE LOOP (thread id " + Thread.currentThread().getId() + ")");
