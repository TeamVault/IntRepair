...
public class RaceChecker {
	...
	public static String raceConditionMessage = "Race condition";
	...
	public void findRace(ArrayDeque<IMTANode> interleaving) {
		...
		for (Integer i = 0; i < interleaving.size(); i++) {
			...
			if (node instanceof MTUseSharedNode) {
				IBinding var = ((MTUseSharedNode) node).getName()
						.resolveBinding();
				openUses.get(node.getPThreadNr()).put(var, i); // store index of
																// node nr of
																// last read
				...
				if (openReadOther) {
					...
					for (int f = i; f < interleaving.size(); f++) {
						...
						if ((fnode instanceof MTDefSharedNode)
								&& (fnode.getPThreadNr() == otherThread)
								&& (((MTDefSharedNode) fnode).getBinding()
										.equals(var))) {
							futureWriteOther = true;
							// race found: overlapping reads and writes
							IBasicBlock bb = ((MTDefSharedNode) node)
									.getBlock();
							...
							String problemMessage = new String(
									raceConditionMessage);
							String problemDescription = new String(
									" overlapping define by thread "
											+ node.getPThreadNr()
											+ " and thread " + otherThread);
							wp.queueProblem(problemid, file, startline,
