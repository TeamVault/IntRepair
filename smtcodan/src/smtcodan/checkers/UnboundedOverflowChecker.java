...
public class UnboundedOverflowChecker implements IPathObserver {
	...
	public static String unboundedOverflowMessage = "Overflow in loop";
	...
	public void checkUnboundedOverflow(PathExplorer pe, IBasicBlock decnode,
			int stemLength, boolean firstClosed) throws InfiniteLoopException {
		...
		for (int i = 0; i < loopSrcInts.size(); i++) {
			...
			xeqs.append("(assert (= xp" + i + " "
					+ loopSrcInts.get(i).getSSAName() + " ))\n");
		}
		// 2. U, u
		...
		if (dims == 1) {
			...
			equiv1.append(") (implies "
					+ XtoT(Ux.toString())
					+ " "
					+ assertsToAndsInsetNormalTp(loopPosts.toString(),
							loopSrcInts) + " ) )) \n");
			for (int i = 0; i < prevInts.size(); i++) {
			...
			equiv2.append(") (implies "
					+ assertsToAndsInsetNormalTp(loopPosts.toString(),
							loopSrcInts) + " " + XtoT(Ux.toString()) + " "
					+ " ) ))\n");
			for (int i = 0; i < prevInts.size(); i++) {
			...
			equiv3.append(") (implies " + Sharpen(XtoT(Gx.toString())) + " "
					+ assertsToAndsInsetNormalT(loopPres.toString(), prevInts)
					+ " ) )) \n");
			// "<="
			...
			equiv4.append(") (implies "
					+ assertsToAndsInsetNormalT(loopPres.toString(), prevInts)
					+ " " + Sharpen(XtoT(Gx.toString())) + " ) )) \n");
			...
			for (int j = 0; j < dims; j++) {
				...
				line.append(") (implies " + XtoT(Uxj.get(j).toString()) + " "
						+ loopPosts.get(j) + " ) )) \n");
				line2.append(") (implies " + loopPosts.get(j) + " "
						+ XtoT(Uxj.get(j).toString()) + " ) )) \n");
				equiv.add(line);
				equiv.add(line2);
			}
		...
		String basicU = stemdefs.toString() + loopdefs + xdefs + Uudefs
				+ stemeqs + loopeqs + xeqs + Uueqs;
		String basicG = xdefs.toString() + Ggdefs + "(assert "
				+ Sharpen(Gx.toString()) + ")\n";
		String basicset = stemdefs.toString() + loopdefs + xdefs + Uudefs
				+ Ggdefs + stemeqs + loopeqs + xeqs + Uueqs + Ggeqs;
		...
		for (int j = 0; j < dims; j++) {
			...
			for (int i = 0; i < dims; i++) {
				...
				token = ltokenz.nextToken(")");
				token = token.replace("(", "").replace(" ", ""); // brackets for
																	// negative
																	// numbers...
																	// and white
																	// spaces
		...
		if (dims == 1) {
			...
			evec.append(new String(
					"(assert (= (+ (* U00 v0) (* U01 v1) ) (* lambda v0 ) ))\n"));
			evec.append(new String(
					"(assert (= (+ (* U10 v0) (* U11 v1) ) (* lambda v1 ) ))\n"));
			// evec not zero vector
		...
		query = evecdef.toString() + evaluedefs + basicset + normcoeffs
				+ evalue + evec + evec_unboundG + checkSat + exit;
		reply = sv.queryReply(query).toString();
		...
		if (dims == 1) {
			u_evec.append(new String(
					"(assert (= (* U00 u0) (* lambdau u0 ) ))\n"));
			// evec not zero vector
			u_evec.append("(assert (> (abs u0) 0))\n");
		} else if (dims == 2) {
			u_evec.append(new String(
					"(assert (= (+ (* U00 u0) (* U01 u1) ) (* lambdau u0 ) ))\n"));
			u_evec.append(new String(
					"(assert (= (+ (* U10 u0) (* U11 u1) ) (* lambdau u1 ) ))\n"));
			// evec not zero vector
		...
		query = u_evaluedefs + basicset + normcoeffs + u_evalue + u_evec
				+ u_evec_unboundG + checkSat + exit;
		reply = sv.queryReply(query).toString();
		if (reply.contains("unsat")) {
		...
		if (dims == 1) {
			...
			uxlindep.append(" 0 ))\n");
			query = u_evaluedefs + basicset + normcoeffs + u_evalue + u_evec
					+ u_evec_unboundG + uxlindep + checkSat + exit;
			reply = sv.queryReply(query).toString();
			if (reply.contains("unsat")) {
				u_x_lindep = false;
			} else {
		...
		if (U_evec_unbound_G && u_x_lindep) {
			...
			query = evecdef.toString() + evaluedefs + basicset + normcoeffs
					+ evalue + evec + evec_unboundG + checkSat + getEval + exit;
			reply = sv.queryReply(query).toString();
			String parts[] = reply.split("lambda");
			String part = parts[1].replace(")", "").replace(" ", "")
					.replace("\n", "");
			lambda = Integer.parseInt(part);
			// FIXME multi-dim!
			String geom = new String(
					"(assert (>= (* (- lambda 1) (abs x0) )  (* lambda (abs u0) ) ))\n");
			query = evecdef.toString() + evaluedefs + basicset + normcoeffs
					+ evalue + evec + evec_unboundG + geom + checkSat + exit;
			reply = sv.queryReply(query).toString();
			if (reply.contains("unsat")) {
				case4 = false;
			} else {
		...
		if (founderror) {
			// found error
			String problemMessage = new String(unboundedOverflowMessage);
			...
			System.out
					.println(" PathVal: found sat unbounded (loop) overflow, (thread "
	...
	public String removeImps(SymVarSSA ssa) {
		...
		for (SymVarSSA dep : ssa.getDependencies()) {
			if (dep.getOrigName() instanceof ImpVarName) {
				expformula = expformula.replace(dep.getSSAName(),
						removeImps(dep));
			}
		}
	...
	public String assertsToAndsInsetNormalTp(String input,
			ArrayList<SymVarSSA> loopSrcInts) {
		StringBuffer filtered = new StringBuffer();
		StringTokenizer tokenz = new StringTokenizer(input, "\n");
		String line = tokenz.nextToken("\n");
		for (int i = 0; i < loopSrcInts.size(); i++) {
	...
	public String assertsToAndsInsetNormalT(String input,
			ArrayList<SymVarSSA> loopSrcInts) {
		StringBuffer filtered = new StringBuffer();
		StringTokenizer tokenz = new StringTokenizer(input, "\n");
		String line = tokenz.nextToken("\n");
