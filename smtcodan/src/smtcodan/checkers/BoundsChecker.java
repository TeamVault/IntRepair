...
public class BoundsChecker implements IMemObserver {
	...
	/** The problem message upper. */
	public static String problemMessageUpper = "Upper array bound violation";

	/** The problem message lower. */
	public static String problemMessageLower = "Lower array bound violation";
	...
	public void updateWrite(SymPointerSSA access, IFile file,
		...
		if (!reply.contains("unsat")) {
			...
			problemMessage = new String(problemMessageUpper);

			problemDescription = new String(" trace: " + newline
					+ ps.getTrace() + newline + " verification condition: "
					+ newline + verifyUpper + reply + newline);
			foundError = true;
			linearSystem = logicDef + sliceDefs + sliceEqs + assertUpper;
		}
		if (foundError) {
			...
			// problemDescription);
			if (!Config.disableBufferOverflowChecker)
				pe.reportProblem(problemid, file, startline, problemMessage,
						problemDescription, new SMTConstraintObject(bufferSize,
								linearSystem, offset), ps.getNvlist());
	...
	public void updateWrite(SymVarSSA target, String offset, IFile file,
		...
		if (!reply.contains("unsat")) {
			problemMessage = new String(problemMessageLower);

			problemDescription = new String(" trace: " + newline
					+ ps.getTrace() + newline + " verification condition: "
					+ newline + verifyLower + reply + newline);
			foundError = true;
			linearSystem = logicDef + sliceDefs + sliceEqs + assertLower;
		}
