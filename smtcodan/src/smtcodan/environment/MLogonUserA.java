 ...
 */
public class MLogonUserA implements IFctModel {
	 ...
	 */
	public MLogonUserA(Interpreter ps) {
		this.ps = ps;
		...
		if (commentMap == null) {
			...
			if (comment == null) {
				MyLogger.log_parser("LogonUserA comment is null");
			} else {
	...
	public static String getLibrarySignature() {
		...
		return "int LogonUserA(char *username, char *domain, char password, int LOGON32_LOGON_NETWORK, int LOGON32_PROVIDER_DEFAULT, HANDLE pHandle);";
	}
	...
	public SymFunctionReturn exec(SymFunctionCall call) {
		...
		SymPointerSSA isp2 = (SymPointerSSA) ps
				.getLocalOrigSymVar(plist.get(2)).getCurrentSSACopy();
		...
		// CWE-259 and CWE-534 use this function model
		...
		return new SymFunctionReturn(isp.getCurrentSSACopy());
	}
