 ...
 */
public class RefactoringFinderIntegerOverflow extends AbstractChecker implements
		ActionListener {
	...
	public QuickFixIntegerOverflow quickFixBounds;
	...
	/**
	 * Instantiates a new propagation. placeHolder
	 * 
	 ...
	 *            the refactorings
	 */
	public RefactoringFinderIntegerOverflow(final Map propagationMap,
		...
		this.quickFixBounds = new QuickFixIntegerOverflow();
	...
	public static void setInitialBugFile(IASTFileLocation initialBugFile) {
		RefactoringFinderIntegerOverflow.initialBugFile = initialBugFile;
	}
	 ...
	 *            the quick fix bounds
	 */
	public void addRefactroingToRefactoringList(Map propagationMap,
			QuickFixIntegerOverflow quickFixIntegerOverflow) {
			...
			for (int i = 0; i < ((ArrayList<PathObject>) pairs.getValue())
				...
				if (((ArrayList<PathObject>) pairs.getValue()).get(i)
					...
					if (walkthroughList.size() >= 1) {
						...
						refactoringsList.add(quickFixIntegerOverflow
	...
	public void removeDuplicatesAndConcatenate() {
		// remove the quick fixes duplicates
		/*
		 * int count = RefactoringFinderIntegerOverflow.refactoringsList.size();
		 * for (int t = 0; t < count; t++) { for (int j = t + 1; j < count; j++)
		 * { if (RefactoringFinderIntegerOverflow.refactoringsList .get(t)
		 * .getReplaceStatementNode() .getRawSignature() .toString()
		 * .equals(RefactoringFinderIntegerOverflow.refactoringsList .get(count
		 * - 1).getReplaceStatementNode() .getRawSignature().toString())) { //
		 * remove the duplicate
		 * RefactoringFinderIntegerOverflow.refactoringsList.remove(j); count--;
		 * } } }
		 */
	...
	public void runQuickFixSearchingAlgorithm(IASTNode in,
		...
		// set the replaceable label
		quickFixIntegerOverflow.setReplaceStringNew(in.getRawSignature()
				.toString());
		// set the cons obj
		...
		quickFixIntegerOverflow
				.setTheConstraintVarAndValueFromMarker(AssertInformationExposure.INFORMATION_EXPOSURE_QUICK_FIX_MESSAGE);
		// set the string where the bug will be detected
		quickFixIntegerOverflow.setReplaceStringOld(in.getRawSignature()
				.toString());
		quickFixIntegerOverflow
				.setTheConstraintVarAndValueFromMarker("Quick-Fix Location");
		// set the Initial bug location file
		...
		bugRefactoring = new Refactoring();
		refactoringsList
				.add(quickFixIntegerOverflow.createLocalRefactoringObject(
						bugRefactoring, in, this.workPool));
