...
public class SmProcessor extends ASTVisitor implements IFunctionCallCollector {
			...
			if (dspec instanceof IASTElaboratedTypeSpecifier) {
				...
				case IASTElaboratedTypeSpecifier.k_struct: {
					if (type instanceof ICompositeType) {
						...
						// ArrayList<SymVarOrig>fields = sc.getFields();
						//
						// for (int i = 0; i<fields.size(); i ++)
						// {
						// String str = fields.get(i).toString();
						// var.addField(fields.get(i), str);
						// }

						System.out.println();
						// AttrSymComposite asymc = new AttrSymComposite(sc);
						// attrList.add(declarator, asymc);
					}
					break;
				}
				}
				...
				if (type instanceof ICompositeType) {
					...
					case "eChar": {
						ps.declareLocal(eSymType.SymPointer, iName);
				...
				if (declarator instanceof IASTArrayDeclarator) {
					...
					if (ps.existsLocalOrigSymVar(iName)) {
						...
						// /***************************************
						// sb = new SymArrayOrig(iName, sb_size);
						// sb.setElemType(symtype);
						// ps.declareLocal(sb);
						// /***************************************
	...
	public int leave(IASTDeclSpecifier dspec) {
		...
		if (dspec instanceof IASTNamedTypeSpecifier) {
			...
			if (type instanceof ICompositeType) {
				...
				AttrSymComposite asymc = new AttrSymComposite(sc);
				attrList.add(dspec, asymc);
				if (dspec instanceof CASTTypedefNameSpecifier) {

				} else {
					ps.declareLocal(sc.getSymType(), (IName) dspec);
				}
				...
				if (type instanceof ICompositeType) {
					...
					for (IField field : fields) {
						...
						if (ftype instanceof IArrayType) {
							...
							// ps.declareLocal(sao.getSymType(), newfieldname);
							// sc.addField(sao, fname);
							...
							for (IField f : fields2) {
								...
								if (ftype2 instanceof ITypedef) {
									...
									if (ftypeInnerLoop instanceof ITypedef) {
										...
										ps.declareLocal(so.getSymType(),
												newfieldname);
										sc.addField(so, fname2);
									}
								}
							}
							...
							// so = (SymVarOrig)
							// ps.declareLocal(so.getSymType(), null);
							// sc.addField(so, fname);
						}
					}
					// service added here
	...
	public int leave(IASTExpression expr) {
		...
		if (expr instanceof IASTFunctionCallExpression) {
			...
			// vasantha added for not external calls
			fcnames.add(fname);
			// IFunction binding = null;
			// // vasantha added from info flow
			// IASTInitializerClause inicllist2[] =
			// ((IASTFunctionCallExpression) expr)
			// .getArguments();
			...
			// // SymIntOrig si = new SymIntOrig(orig_name);
			// // ps.declareGlobal(si.getSymType(), null);
		...
		if (expr instanceof IASTLiteralExpression) {
			...
			case (IASTLiteralExpression.lk_char_constant): { // TODO
				...
				// SymIntOrig si = new SymIntOrig(iname);
				// ps.declareLocal(si.getSymType(), null);
				// ssa_copy =
				// ps.getLocalOrigSymInt(iname).getCurrentSSACopy();
				// Integer charval =
				// Character.getNumericValue(sval.charAt(0));
				//
				// String charNumForm = null;
				// if (charval < 0) {
				// // filter negatives
				...
				// charNumForm = new String(Integer.toString(charval));
				// }
				// avoid setting a formula from empty space
				// if (sval.equals("' '")) {
				// break;
				// } else {
				// ssa_copy.setFormula(new String("(assert (= "
		...
		if (expr instanceof IASTBinaryExpression) {
			...
			AttrSymVar aSymVarLeft = (AttrSymVar) attrList.getAttribute(
					operandLeft, "symvar");
			if (aSymVarLeft == null || aSymVarLeft.getSymVar() == null) {
				return 0;
			}
			...
			if (aSymVarRight == null || aSymVarRight.getSymVar() == null) {
				return 0;
			}
			...
			// this is used for CWE-190 to store the current upper bound value
			...
			if (binop == IASTBinaryExpression.op_assign) {
				...
				// else if (symVarLeft.getESymType() == eSymType.SymInt
				// && symVarRight.getESymType() == eSymType.SymInt) {
				// if (symVarLeft.isInitialized()) {
				// symVarLeft = ps.ssaCopy(symVarLeft.getOrig());
				// System.out.println("init: " + symVarLeft.getOrigName());
				// System.out
				// .println("init: " + symVarRight.getOrigName());
				//
				// System.out.println("init dep size: "
				// + symVarLeft.getDependencies().size());
				// System.out.println("expr: " + expr.getRawSignature());
				//
				// }
				// System.out.println("expr2: " + expr.getRawSignature());
				// System.out.println("init2: " + symVarLeft.getOrigName());
				// System.out.println("init2: " + symVarRight.getOrigName());
				// String formula = new String("(assert (= "
				...
				if ((symVarLeft.getESymType() == eSymType.SymInt)
					...
					case SymInt: {
						...
						SymVarSSA opOther;
						if (symVarLeft.getESymType() == eSymType.SymPointer) {
							opPointer = (SymIntSSA) symVarLeft;
							opOther = symVarRight;

						} else {
							opPointer = (SymIntSSA) symVarRight;
							opOther = symVarLeft;
						}
			...
			} else if (binop == IASTBinaryExpression.op_lessThan) {
				SymBoolSSA res_ssa = (SymBoolSSA) ps.declareLocal(
						eSymType.SymBool, null);
				SymBoolOrig res = (SymBoolOrig) res_ssa.getOrig();
				...
				if ((symVarLeft.getESymType() == eSymType.SymInt)
						&& (symVarRight.getESymType() == eSymType.SymPointer)) {
					System.out.println("expr:: " + expr.getRawSignature());
					SymPointerSSA sps = (SymPointerSSA) symVarRight;
					eSymType targetsymtype = symVarLeft.getESymType();
					String formula = "";
					switch (targetsymtype) {
					case SymInt: {
						// new array version necessary
						SymPointerSSA oldssa = (SymPointerSSA) sps;
						SymPointerOrig sao = ps.getLocalOrigSymPointer(oldssa
								.getOrigName());
						SymPointerSSA newssa;
						...
						&& (symVarRight.getESymType() == eSymType.SymInt)) {
					System.out.println("expr:: " + expr.getRawSignature());
					SymIntSSA sps = (SymIntSSA) symVarRight;
					eSymType targetsymtype = symVarLeft.getESymType();
					String formula = "";
					switch (targetsymtype) {
					case SymInt: {
				...
				BigInteger res;
				String formula = "";
				if (div2.compareTo(new BigInteger("0")) == 1
						|| div2.compareTo(new BigInteger("0")) == -1
						|| div1.compareTo(new BigInteger("0")) == 1
						|| div1.compareTo(new BigInteger("0")) == -1) {
					res = div1.divide(div2);
					// formula = "(assert (= " + ssa.getSSAName() + "( / "
					// + symVarLeft.getSSAName() + " "
					// + symVarRight.getSSAName() + " )))";

					formula = formula + "(assert (= " + ssa.getSSAName() + " "
							+ res + " ))";

					System.out.println("div1 and div2: " + div1.toString()
							+ " " + div2.toString());
				} else if (div1.compareTo(new BigInteger("0")) == 0
						|| div2.compareTo(new BigInteger("0")) == 0) {
				...
				// null);
				// SymVarOrig svo = ssa.getOrig();
				// int div1 = ps.getConcreteInt(symVarLeft);
				...
				if (etype instanceof IBasicType) {
					...
					case eChar: {
						ssa = (SymVarSSA) ps.declareLocal(eSymType.SymPointer,
								null);
						break;
					}
					case eInt: {
